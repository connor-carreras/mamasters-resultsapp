q_medals_list = """
select 
  sum(case when race_rank_by_gender_class <= 3 then 1 else 0 end) as total_medals,
  sum(case when race_rank_by_gender_class = 1 then 1 else 0 end) as total_gold,
  sum(case when race_rank_by_gender_class = 2 then 1 else 0 end) as total_silver,
  sum(case when race_rank_by_gender_class = 3 then 1 else 0 end) as total_bronze
  from results_by_class_vw
...
from results_vw where racekey='{selected_option}') order by 2, 3
"""
...
run2,
total,
worldcup_points_by_gender_class,
race_points
from results_by_class
where racekey='{selected_option}' and insert_ts = (select max(insert_ts) from results_by_class where racekey='{selected_option}')
and gender = '{gender}'
and class = '{raceclass}'
...
, ranked_results as (
select *,
rank() over (partition by racekey, gender, class order by total) as race_rank_by_gender_class
from results_with_dsq
)

, wc_points as (
select r.*, 
ifnull(p.points,0) as worldcup_points_by_gender_class
from ranked_results r 
left join worldcup_points p 
on r.race_rank_by_gender_class = p.place
)

, corrected_points as (
select raceseries, division, mountain, racekey, racetype, racedate, bib, name, ussanumber, class, gender, run1, run2, total, ingest_ts, season, run1_dsq, run2_dsq, run1_dnf, run2_dnf,
case when total is null then null else race_rank_by_gender_class end as race_rank_by_gender_class,
case when total is null then 0 else worldcup_points_by_gender_class end as worldcup_points_by_gender_class
from wc_points
)
